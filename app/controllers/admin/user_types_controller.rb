...
class Admin::UserTypesController < Admin::ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @user_type.save
         format.html { redirect_to(admin_user_types_path, :notice => '成功') }
  ...
  def update
    respond_to do |format|
      if @user_type.update(user_type_params)
        format.html { redirect_to(admin_user_types_path, :notice => '更新成功') }
  ...
  private
    ...
    def user_type_params
      params.require(:user_type).permit(:name, :remark)
    end
end
